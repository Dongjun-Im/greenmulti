...
데스크톱 유틸리티 v1.3. 초록등대 동호회의 일반 회원 이상이면 사용 가능.
...
- `nas.py`: 초록등대 자료실 rclone mount 엔진 + WinFSP 의존. Windows
  WebClient 기반 구현은 v1.3 에서 MRxDAV 비호환으로 제거
- `credentials.py`, `login_dialog.py`, `authenticator.py`: 인증
  (green_auth 패키지와 별도로 한 번 더 존재 — 번들 구성 흔적)
- `config.py`: 경로, 소리샘 URL, 앱 메타 (v1.3.0, 2026-04-22)
- `green_auth/`: 공용 인증 패키지 **번들 복사본** (원본 리포는 별도)
- `bin/rclone.exe`: 번들된 rclone 바이너리 (WebDAV→드라이브 문자 마운트)
- `installer_deps/winfsp.msi`: Inno Setup 이 자동 설치하는 WinFSP
  파일시스템 드라이버
- `installer.iss` / `version_info.txt`: 배포 빌드 관련 리소스
...
- 기타: Ctrl+N(NAS 연결), Ctrl+J(다운로드 상태), Ctrl+L(로그아웃)
- 테마/글꼴: F7(설정 통합), F6/Shift+F6(다음/이전), Ctrl++/-/0
...
- `build/`, `dist/`, `installer_out/`, `__pycache__/`, `.venv*/`
